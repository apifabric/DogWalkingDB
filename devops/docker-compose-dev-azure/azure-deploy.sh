...
resourcename="dogwalkingdb"  # lower case, only
resourcegroup="dogwalkingdb_rg"
...
# git clone https://github.com/apilogicserver/dogwalkingdb.git
# cd dogwalkingdb
...
az group create --name dogwalkingdb_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group dogwalkingdb_rg --sku S1 --is-linux
az container create --resource-group dogwalkingdb_rg --name dogwalkingdb --image apilogicserver/dogwalkingdb:latest --dns-name-label dogwalkingdb --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//dogwalkingdb.westus.azurecontainer.io:5656'
